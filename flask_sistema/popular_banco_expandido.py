...
def carregar_dados_json():
    ...
    try:
        ...
        print(f"✅ Carregados dados de veículos para {len(dados['veiculos'])} diretorias")
    except FileNotFoundError:
    ...
    try:
        ...
        print(f"✅ Carregados dados de supervisão para {len(dados['supervisao'])} regiões")
    except FileNotFoundError:
...
def popular_diretorias(dados):
    ...
    for nome, info in dados['diretorias'].items():
        ...
        diretoria = Diretoria(
            ...
            total_veiculos=len([v for v in dados['veiculos'].get(nome, {}).get('veiculos_por_tipo', {}).values() if v.get('quantidade', 0) > 0])
        )
...
def popular_escolas(dados):
    ...
    for nome_diretoria, info_diretoria in dados['diretorias'].items():
        ...
        for escola_info in info_diretoria.get('escolas', []):
            ...
            if codigo_mec:
                escola_existente = Escola.query.filter_by(codigo_mec=codigo_mec).first()
                if escola_existente:
                    continue
...
def popular_supervisores(dados):
    ...
    for regiao, info in dados['supervisao'].items():
        # Verificar se já existe
        supervisor_existente = Supervisor.query.filter_by(regiao=regiao).first()
