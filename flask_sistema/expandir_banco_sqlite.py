...
def popular_estatisticas_sistema(conn, dados):
    ...
    cursor.execute("DELETE FROM estatisticas_sistema WHERE data_referencia = ?", (date.today(),))
...
def main():
    ...
    try:
        ...
        print(f"✅ Dados carregados: {len(dados['diretorias'])} diretorias, {len(dados['veiculos'])} veículos, {len(dados['supervisao'])} supervisores")
