...
def atualizar_dados_consolidados():
    ...
    for diretoria in diretorias:
        ...
        if nome in siglas_oficiais:
            ...
            nome_alt = nome.replace('BRAGANCA', 'BRAGANÇA').replace('FERNANDOPOLIS', 'FERNANDÓPOLIS')
            if nome_alt in siglas_oficiais:
                diretoria['sigla'] = siglas_oficiais[nome_alt]
                diretorias_atualizadas += 1
    
    print(f"   ✅ {diretorias_atualizadas} diretorias atualizadas com siglas oficiais")
...
def criar_banco_sqlite_final(diretorias, escolas, tipos_escola):
    ...
    # Criar conexão
    conn = sqlite3.connect('dados/consolidados/banco_completo_91_diretorias.db')
    cursor = conn.cursor()
    ...
    for codigo, tipo_info in tipos_escola.items():
        qtd_escolas = len([e for e in escolas if e['tipo_escola_codigo'] == int(codigo)])
    ...
    estatisticas = [
        ...
        ('total_diretorias', len(diretorias), 'Total de diretorias/unidades regionais'),
        ('total_tipos', len(tipos_escola), 'Total de tipos de escola'),
        ('escolas_indigenas', len([e for e in escolas if e['tipo_escola_codigo'] == 10]), 'Escolas indígenas'),
        ('escolas_quilombolas', len([e for e in escolas if e['tipo_escola_codigo'] == 36]), 'Escolas quilombolas'),
        ('escolas_assentamento', len([e for e in escolas if e['tipo_escola_codigo'] == 31]), 'Escolas de assentamento'),
        ('escolas_regulares', len([e for e in escolas if e['tipo_escola_codigo'] == 8]), 'Escolas regulares'),
    ]
    ...
    cursor.execute('CREATE INDEX idx_escolas_tipo ON escolas(tipo_escola_codigo)')
...
"""
    ...
    diretorias_ordenadas = sorted(stats_diretorias.items(), key=lambda x: x[1]['total'], reverse=True)
...
def main():
    ...
    try:
        ...
        gerar_relatorio_final(diretorias, escolas, tipos_escola, siglas_oficiais)
        ...
        print(f"   📁 Banco SQLite criado: dados/consolidados/banco_completo_91_diretorias.db")
