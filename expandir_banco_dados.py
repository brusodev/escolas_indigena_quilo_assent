...
def carregar_dados_completos():
    ...
    try:
        ...
        with open('dados_escolas_atualizados.json', 'r', encoding='utf-8') as f:
            dados['escolas'] = json.load(f)
        print(
            f"✅ Carregadas {len(dados['escolas'])} escolas (arquivo alternativo)")
    ...
    try:
        ...
        print(
            f"✅ Carregados dados de veículos para {len(dados['veiculos']['diretorias'])} diretorias")
    except FileNotFoundError:
    ...
    try:
        ...
        print(
            f"✅ Carregados dados de supervisão para {len(dados['supervisao'])} unidades")
    except FileNotFoundError:
    ...
    try:
        df_diretorias = pd.read_excel(
            'dados/excel/diretorias_com_coordenadas.xlsx')
        ...
        print(
            f"✅ Carregadas coordenadas de {len(dados['diretorias_coordenadas'])} diretorias")
...
def criar_veiculos_detalhados(dados):
    ...
    for diretoria, info in dados['veiculos']['diretorias'].items():
        veiculos_detalhados[diretoria] = {
            ...
            # Tem tração 4x4
            'necessidade_especial': info.get('s2_4x4', 0) > 0,
...
def gerar_relatorio_completo(dados, diretorias_completas, veiculos_detalhados):
    ...
    relatorio = {
        ...
        'diretorias_com_mais_escolas': sorted(
            [(nome, info['estatisticas']['total_escolas'])
             for nome, info in diretorias_completas.items()],
            key=lambda x: x[1], reverse=True
        )[:10],
        'diretorias_com_mais_veiculos': sorted(
            [(nome, info['total_veiculos'])
             for nome, info in veiculos_detalhados.items()],
            key=lambda x: x[1], reverse=True
        )[:10]
    }
...
def main():
    ...
    relatorio = gerar_relatorio_completo(
        dados, diretorias_completas, veiculos_detalhados)

    # Salvar dados
    print("\n💾 Salvando dados expandidos...")
    salvar_dados_expandidos(diretorias_completas,
                            veiculos_detalhados, relatorio)
    ...
    print(
        f"   📍 {relatorio['resumo_geral']['total_diretorias']} diretorias com dados completos")
    print(
        f"   🏫 {relatorio['resumo_geral']['total_escolas']} escolas processadas")
    print(
        f"   🚗 {relatorio['resumo_geral']['total_veiculos']} veículos catalogados")
    print(
        f"   👥 {relatorio['resumo_geral']['total_regioes_supervisao']} regiões de supervisão")
