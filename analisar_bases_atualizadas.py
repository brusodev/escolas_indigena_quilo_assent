...
def analisar_bases_dados():
    ...
    if os.path.exists("dados\\json\\dados_escolas_atualizados.json"):
        ...
        recomendacoes.append(
            f"✅ ESCOLAS: dados\\json\\dados_escolas_atualizados.json ({len(escolas)} escolas)")
    ...
    if veiculos_file:
        ...
        if 'diretorias' in veiculos:
            ...
            total_veic = veiculos.get('metadata', {}).get(
                'total_veiculos', 'N/A')
            recomendacoes.append(
                f"✅ VEÍCULOS: {veiculos_file} ({total_dirs} diretorias, {total_veic} veículos)")
    ...
    for excel_file in excel_files:
        if os.path.exists(excel_file):
            ...
            data_mod = datetime.fromtimestamp(stat.st_mtime)
            recomendacoes.append(
                f"✅ EXCEL: {excel_file} ({data_mod.strftime('%d/%m/%Y %H:%M')})")
            break
    ...
    if os.path.exists("dados_veiculos_diretorias.json"):
        ...
        if stat.st_size == 0:
            print(
                "❌ dados_veiculos_diretorias.json está VAZIO - usar backup em dados\\json\\")
