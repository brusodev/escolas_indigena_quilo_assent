...
def verificar_coordenadas():
    ...
    print(
        f"Ambas coordenadas válidas: {(df['DS_LATITUDE'].notna() & df['DS_LONGITUDE'].notna()).sum()}")

    # Verificar algumas escolas com coordenadas
    with_coords = df[(df['DS_LATITUDE'].notna()) & (
        df['DS_LONGITUDE'].notna())].head(3)
    print("\nEscolas com coordenadas (primeiras 3):")
    for _, escola in with_coords.iterrows():
        print(
            f"- {escola['NOMESC']}: ({escola['DS_LATITUDE']}, {escola['DS_LONGITUDE']})")

    # Verificar escolas indígenas especificamente
    indigenas = df[df['TIPOESC'] == 10]
    indigenas_com_coords = indigenas[(indigenas['DS_LATITUDE'].notna()) & (
        indigenas['DS_LONGITUDE'].notna())]
