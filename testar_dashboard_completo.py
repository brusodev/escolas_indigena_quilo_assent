...
def testar_dados_escolas():
    ...
    try:
        ...
        if len(escolas) > 0:
            ...
            campos_obrigatorios = ['name', 'type', 'city', 'diretoria', 'lat', 'lng']
            campos_ok = all(campo in escola_exemplo for campo in campos_obrigatorios)
            
            if campos_ok:
                print("✅ Estrutura dos dados: OK")
                print(f"   📍 Exemplo: {escola_exemplo['name']}")
                print(f"   🏛️ Tipo: {escola_exemplo['type']}")
                print(f"   📍 Coordenadas: ({escola_exemplo['lat']}, {escola_exemplo['lng']})")
            else:
        ...
        print(f"\n📍 COORDENADAS:")
        print(f"   ✅ Válidas: {coordenadas_validas:,} ({coordenadas_validas/len(escolas)*100:.1f}%)")
        print(f"   ❌ Inválidas: {len(escolas)-coordenadas_validas:,}")
        
        return coordenadas_validas > len(escolas) * 0.95  # 95% devem ter coordenadas
