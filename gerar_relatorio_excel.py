...
def criar_aba_metodologia_haversine(wb):
    ...
    ws[f"A{row}"] = ("A Fórmula de Haversine é o método padrão internacional para calcular "
    ...
    certificacao = (f"Este documento certifica que todas as 59 distâncias apresentadas "
    ...
    ws[f"A{row}"].alignment = left_align
    ws[f"A{row}"].fill = PatternFill(start_color="D5F4E6", end_color="D5F4E6", fill_type="solid")
    ws.merge_cells(f"A{row}:D{row}")
        ...
        relatorio_data.append(
            {
                ...
                "Observações": f"Escola {row['Tipo_Escola'].lower()} em zona {row['Zona'].lower()}",
            }
        )
