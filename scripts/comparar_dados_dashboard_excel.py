...
def buscar_escolas_originais():
    ...
    try:
        ...
        set_csv = set(escola.strip().upper() for escola in escolas_csv if pd.notna(escola))
        set_excel = set()
        
        try:
            df_excel = pd.read_excel('distancias_escolas_diretorias_corrigido.xlsx')
            set_excel = set(escola.strip().upper() for escola in df_excel['Nome_Escola'].tolist())
        except:
            pass
...
def main():
    ...
    # Comparar
    faltando_excel, faltando_dashboard = comparar_escolas(escolas_dashboard, escolas_excel)
