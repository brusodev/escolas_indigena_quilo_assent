...
# Ler Excel
df = pd.read_excel('Relatorio_Completo_Escolas_Diretorias.xlsx', sheet_name='Todas as Escolas')
...
for col in df.columns[:3]:
    if df[col].astype(str).str.contains('KOPENOTI', na=False, case=False).any():
        ...
        kopenoti_row = df[df[col].astype(str).str.contains('KOPENOTI', na=False, case=False)]
