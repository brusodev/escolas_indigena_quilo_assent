    ...
    try:
        ...
        for obj in school_objects:
            ...
            if name_match:
                ...
                if any(faltante.upper() in nome.upper() for faltante in escolas_faltantes):
                    escola_data = {
                        'Nome_Escola': nome,
                        ...
                        'DE_Responsavel': diretoria_match.group(1) if diretoria_match else '',
                        'Zona': '',  # Não disponível no dashboard
                        'Latitude_Escola': float(lat_match.group(1)) if lat_match else 0,
                        'Longitude_Escola': float(lng_match.group(1)) if lng_match else 0,
                        'Nome_Diretoria': diretoria_match.group(1) if diretoria_match else '',
                        ...
                        'Distancia_KM': float(distance_match.group(1)) if distance_match else 0
                    }
                    escolas_encontradas.append(escola_data)
                    print(f"✅ Encontrada: {nome} ({escola_data['Cidade_Escola']}) - {escola_data['Distancia_KM']} km")
...
def adicionar_escolas_ao_excel(escolas_data):
    ...
    try:
        ...
        # Verificar KOPENOTI
        kopenoti = df_completo[df_completo['Nome_Escola'].str.contains('KOPENOTI', na=False, case=False)]
...
def main():
    ...
    if df_completo is not None:
        ...
        for i, escola in enumerate(escolas_data, 1):
            print(f"   {i}. {escola['Nome_Escola']} ({escola['Cidade_Escola']}) - {escola['Distancia_KM']} km")
