...
def testar_dados_dashboard():
    ...
    try:
        ...
        response = requests.get(f"{base_url}/dados/json/dados_escolas_atualizados.json")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            total_veiculos = veiculos.get('metadata', {}).get('total_veiculos', 0)
            diretorias_veiculos = len(veiculos.get('diretorias', {}))
            print(f"✅ Veículos: {total_veiculos} total em {diretorias_veiculos} diretorias")
        else:
