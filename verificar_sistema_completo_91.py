...
def verificar_banco_consolidado():
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM diretorias WHERE sigla_oficial IS NOT NULL")
    num_siglas = cursor.fetchone()[0]
    print(f"✅ Diretorias com siglas oficiais: {num_siglas}")

    # Verificar algumas diretorias específicas
    cursor.execute(
        "SELECT nome, sigla_oficial FROM diretorias WHERE sigla_oficial IS NOT NULL LIMIT 5")
...
def testar_api_flask():
    ...
    try:
        ...
        response = requests.get(
            "http://127.0.0.1:5000/api/diretorias-91", timeout=10)
        if response.status_code == 200:
            ...
            com_siglas = sum(
                1 for d in data['diretorias'] if d.get('sigla_oficial'))
